function main()
    ...
    for i in 1:N
        xf[:, i] = reshape(exp.(-50*((X .- c_ensemble[1, i]).^2 + (Y .- c_ensemble[2, i]).^2)), Nx*Ny, 1) 
    end
    ...
    # Observation
    observe_index = 1:25:Nx * Ny
    H = Matrix{Float64}(I, Nx*Ny, Nx*Ny)
    H = H[observe_index, :]
    sigma = 0.01
    ...
    rho = cal_rho(localization_radius, Nx*Ny, gaspari_cohn, Nx, Ny, Lx, Ly)
    # iterate over time
    for i = 1:Nt
        ...
        # propogate the "truth"
        u = forward_euler(u, Nx, Ny, dx, dy, dt, cx, cy, nu);

        #u = reshape(u, Nx, Ny);
    
        # propogate each ensembles through the model for one time step
        for j = 1:N
            temp = forward_euler(xf[:, j], Nx, Ny, dx, dy, dt, cx, cy, nu);
            xf[:, j] = reshape(temp, Nx*Ny, 1);
        end
    
        # create observations based around the truth
        y = H * reshape(u, Nx*Ny, 1); # this can be non-linearized
    
        # Do the analysis
        temp_analysis = BabyKF(xf, y, H, R, infl, rho, observe_index, false);
        temp_analysis_mean = mean(temp_analysis, dims=2);
    
        rms_value = sqrt(((norm(temp_analysis_mean - reshape(u, Nx*Ny,1), 2)).^2 +
            (rms_value^2)*(i-1)*length(reshape(u, Nx*Ny,1)))/(i*length(reshape(u, Nx*Ny,1))));

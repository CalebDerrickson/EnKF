function forward_euler(u, Nx, Ny, dx, dy, dt, cx, cy, nu)
    ...
    u_new[1, :] = u_new[Nx-1, :]; # Left boundary wraps to Nx-1
    u_new[Nx, :] = u_new[2, :]; # Right boundary wraps to 2
    u_new[:, 1] = u_new[:, Ny-1]; # Bottom boundary wraps to Ny-1
    u_new[:, Ny] = u_new[:, 2]; # Top boundary wraps to 2
    return u_new
end